...
def load_matrix(path: Path) -> tuple[tuple[str, ...], tuple[str, ...], tuple[Rule, ...]]:
    ...
    critical = tuple(data.get("critical_paths", ()))
    allowed = tuple(data.get("allowed_surfaces", {}).get("values", ()))
    rules = tuple(
        Rule(
            name=item["name"],
    ...
    if not critical or not allowed or not rules:
        raise GateError("synchronization matrix is incomplete")
    known = set(allowed)
    for rule in rules:
        if rule.primary_surface not in known or not set(rule.requires) <= known:
            raise GateError(f"classifier {rule.name} references an unknown surface")
    return critical, allowed, rules


def classify(path: str, rules: tuple[Rule, ...]) -> tuple[str, tuple[str, ...]]:
    matching = [rule for rule in rules if any(fnmatch(path, p) for p in rule.patterns)]
    if not matching:
...
def load_work_record(path: Path, allowed_surfaces: tuple[str, ...]) -> dict:
    ...
    required_strings = ("title", "primary_flow", "human_decision", "independent_review")
    for key in required_strings:
        if not isinstance(data.get(key), str) or not data[key].strip():
            raise GateError(f"work record field {key} must be a non-empty string")
    ...
    if decision == "Not required" and not isinstance(reason, str) or decision == "Not required" and not reason.strip():
        raise GateError("Not required changelog decision needs a specific reason")
    sync = data.get("synchronization", {})
    assessed = sync.get("assessed_surfaces")
    if not isinstance(assessed, list) or len(assessed) != len(set(assessed)):
        raise GateError("synchronization.assessed_surfaces must be a unique array")
    unknown = set(assessed) - set(allowed_surfaces)
    if unknown:
        raise GateError("work record contains unknown surfaces: " + ", ".join(sorted(unknown)))
    ...
    for surface, explanation in non_applicable.items():
        if surface not in assessed or not isinstance(explanation, str) or not explanation.strip():
            raise GateError(f"invalid non-applicable synchronization record: {surface}")
    return data


def validate(root: Path, repo: Path, base: str, head: str) -> tuple[str, ...]:
    matrix_path = root / "governance/synchronization-matrix.toml"
    ...
    record_changes = [
        c for c in changes
        if c.path.startswith("governance/work-records/") and c.path.endswith(".toml") and c.status == "A"
    ]
    if len(record_changes) != 1:
        errors.append("exactly one new governance/work-records/*.toml file is required")
        return tuple(errors)

    record_path = root / record_changes[0].path
    try:
    ...
    if missing:
        errors.append("work record omits required synchronization surfaces: " + ", ".join(sorted(missing)))

    decision = record["changelog"]["decision"]
    changelog_changed = any(c.path == "CHANGELOG.md" for c in changes)
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Validate PR process and synchronization contract")
    ...
    try:
        errors = validate(args.root.resolve(), args.repo.resolve(), args.base, args.head)
