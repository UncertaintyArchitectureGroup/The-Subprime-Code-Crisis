...
The canonical classification, review status, integration-audit status, verification date, and current repository usage for each source live in [`evidence/SOURCES.md`](evidence/SOURCES.md). Detailed source evaluation lives in evidence briefs under `evidence/`.

A source appearing here is not automatically peer reviewed, independently validated, fully integrated, or strong enough to support every claim attributed to it. Follow the source-registry and evidence-brief links for boundaries, limitations, and verification status.
...
- Publication status: working paper; not peer reviewed.
- [Reviewed evidence brief](evidence/primary/2026-writing-code-vs-shipping-code.md).
- Integration status: see [`evidence/SOURCES.md`](evidence/SOURCES.md).
...
- Publication status: original research report; detailed repository brief pending.
...
- Publication status: original measurement report; detailed repository brief pending.
...
- Publication status: preprint; detailed repository brief pending.
...
- Publication status: original commercial measurement report; methodology and conflict-of-interest brief pending.
...
- Publication status: first-party engineering documentation; documentary brief pending.
...
- Publication status: first-party documentary sources. Exact filing versions and claim mappings must be recorded in documentary briefs before aggregate spending figures are treated as load-bearing.
...
- Publication status: industry survey and analysis; brief pending.
...
- Publication status: secondary link only. Replace with and review the original McKinsey publication before using the figures as load-bearing evidence.
...
- Publication status: practitioner analysis; brief pending.
...
- Publication status: practitioner commentary; use for framing, not primary empirical support.
...
- Publication status: practitioner synthesis; brief pending.
...
- Publication status: practitioner reporting; use for context and hypothesis generation.
...
- Publication status: methodology source used to interpret bottleneck migration, local optimization, and inventory. It is not direct evidence of AI-tool effects.

## Maintenance rule

When adding, processing, or updating a source:

1. select the correct flow in [`AGENTS.md`](AGENTS.md);
2. register or confirm the source in `evidence/SOURCES.md`;
3. complete or update its evidence brief;
4. complete the repository-wide integration audit;
5. correct report, README, reference, and protocol implications where required;
6. synchronize this bibliography last;
7. mark `Integration audit = Verified` only after all required corrections exist on the default branch.

This bibliography does not duplicate the source-state database. See [`evidence/SOURCES.md`](evidence/SOURCES.md) for current status and [`AGENTS.md`](AGENTS.md) for the complete workflow.
